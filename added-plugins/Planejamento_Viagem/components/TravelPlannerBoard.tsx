...
const BOTTOM_PADDING_PX = 44;            // espaço extra p/ não “cortar” o último card
const TOP_STRIPE_PX     = 12;            // faixa transparente no topo do card

// Durações padrão de base
const BASE_START_MIN = 60;               // início do dia
const BASE_END_MIN   = 120;              // término do dia (default)
const BASE_MID_MIN   = 120;              // base intermediária
// POI
const POI_CARD_MIN   = 60;
...
/* ===================== Distância/ETA ===================== */
function isValidCoord(c){ return Array.isArray(c)&&c.length===2&&Number.isFinite(c[0])&&Number.isFinite(c[1]); }
...
const etaFromKm=(km,kmh=CRUISING_KMH_DEFAULT)=>Math.max(5,Math.round((km/Math.max(10,kmh))*60));
...
function notifyMapFull(board, dayColors, onRebuildRoutes){
  ...
  const viagens = board.map((d,i)=>{
    ...
    for(let k=0;k<d.pontos.length-1;k++){
      ...
      legs.push({
        ...
        eta_min: eta,
        style:{ halo:{color,weight:8,opacity:.25,lineCap:'round'},
                main:{color,weight:5,opacity:.60,lineCap:'round'} }
      });
    }
    return {
  ...
  (window||{}).TT_TRIP_TOTALS={ total_km: Math.round((TRIP_METERS/1000)*10)/10, total_min: Math.round(TRIP_MIN) };

  const w=(window||{});
  w.TT_VIAGENS=viagens;
  const dayColorMap={}; board.forEach((d,i)=>dayColorMap[d.data]=dayColors[i]);
  w.TT_DAY_COLORS=dayColorMap;
  w.TT_ROUTE_COLOR_OF_DAY=(iso)=>dayColorMap[iso]||'#3388ff';

  const flat=[]; for(const v of viagens) for(const l of v.legs) flat.push({day:v.day,coords:[l.a,l.b],style:l.style});
  w.TT_ROUTES=flat;

  const pins=[];
  board.forEach((d,i)=>{
    const color=dayColors[i];
    d.pontos.forEach((p)=>{ if(isValidCoord(p?.coordinates)) pins.push({label:p.label,coords:p.coordinates,day:d.data,color,tipo:p.tipo}); });
  });
  w.TT_PINS=pins;
  w.__TT_LAST_DRAW={flat,pins};

  // Tenta desenhar imediatamente e re-tenta até os drawers existirem
  ...
  const tryDraw=(attempt=0)=>{
    ...
    if(!(typeof drawR==='function' && typeof drawP==='function') && attempt<80){
      w.__TT_DRAW_RETRY=setTimeout(()=>tryDraw(attempt+1),200);
    }
  };
  tryDraw();
...
function sameCoords(a,b){
  return !!a&&!!b&&a.coordinates?.[0]===b.coordinates?.[0]&&a.coordinates?.[1]===b.coordinates?.[1];
}
...
function ensureStartIsBase(d){
  ...
  if(d.pontos[0]?.tipo!=='base'){
    ...
    if(firstBase){
      d.pontos.unshift({ ...firstBase, fixo:true, id:`base-start-${d.data}`, tempo:BASE_START_MIN, startAtMin:0 });
    }
  }else{
    d.pontos[0]={ ...d.pontos[0], fixo:true, id:`base-start-${d.data}`, tempo:BASE_START_MIN, startAtMin:0 };
  }
}
...
function ensureEndIsBase(d){
  ...
  if(last?.tipo!=='base'){
    ...
    if(lastBase){
      d.pontos.push({
        ...lastBase, fixo:false, id:`base-end-${d.data}`,
        tempo: BASE_END_MIN,
    ...
    d.pontos[d.pontos.length-1]={
      ...last, fixo:false, id:`base-end-${d.data}`,
      tempo: Math.max(BASE_END_MIN, last.tempo ?? BASE_END_MIN),
      startAtMin: typeof last.startAtMin==='number'
        ? last.startAtMin
        : Math.max(0, DAY_END_MIN - DAY_START_MIN - Math.max(BASE_END_MIN, last.tempo ?? BASE_END_MIN))
    };
  }
}

/** Fim do dia i = início do dia i+1 (base igual, início 08:30) */
  ...
  for(let i=0;i<novo.length-1;i++){
    ...
    if(!first || first.tipo!=='base' || !sameCoords(first,endBase)){
      ...
      novo[i+1].pontos.unshift({
        ...endBase, fixo:true, id:`base-start-${novo[i+1].data}`, tempo:BASE_START_MIN, startAtMin:0
      });
    }else{
      novo[i+1].pontos[0]={ ...first, fixo:true, id:`base-start-${novo[i+1].data}`, tempo:BASE_START_MIN, startAtMin:0 };
    }
...
/** Sequenciamento respeitando startAtMin e salvando km do deslocamento */
function scheduleDay(dia){
  ...
  for(let i=0;i<dia.pontos.length;i++){
    ...
    const travelMin=i>0 ? minutesBetween(prev,cur) : 0;
    const travelKm = (i>0 && isValidCoord(prev?.coordinates) && isValidCoord(cur?.coordinates))
      ? Math.round((haversineMeters(prev.coordinates,cur.coordinates)/1000)*10)/10
      : 0;
    ...
    const baseDefault = cur.tipo==='base' ? BASE_MID_MIN : POI_CARD_MIN;
    const d = Math.max(RESIZE_STEP_MIN, Math.round((cur.tempo ?? baseDefault)/RESIZE_STEP_MIN)*RESIZE_STEP_MIN);

    const start=clock;
    const end=Math.min(DAY_END_MIN, start+d);

    items.push({ ...cur, start, end, travelBefore:travelMin, travelKmBefore:travelKm });
...
/** Cola a base final até 00:00 (mínimo 120 min) e sem sobrepor anteriores */
function pinEndBaseToMidnight(scheduled){
  if(!scheduled.length) return scheduled;
  let idx=-1;
  for(let i=scheduled.length-1;i>=0;i--) if(scheduled[i].tipo==='base'){ idx=i; break; }
  if(idx<0) return scheduled;

  const prev=scheduled[idx-1];
  const b={...scheduled[idx]};
  const desiredDur=Math.max(BASE_END_MIN, Math.max(RESIZE_STEP_MIN,Math.round((b.tempo ?? BASE_END_MIN)/RESIZE_STEP_MIN)*RESIZE_STEP_MIN));

  let start=DAY_END_MIN - desiredDur;
  if(prev) start=Math.max(start, prev.end);

  b.start=start; b.end=DAY_END_MIN;
  b.id = b.id?.startsWith('base-end-') ? b.id : `base-end-${(b.data||'')}`;

  const out=[...scheduled]; out[idx]=b; return out;
}
...
function minutesBetween(prev,next){
  try{
    ...
    const v=idx[key]; if(typeof v==='number'&&v>=0) return v;
  }catch{}
  return Math.max(5, etaFromKm(haversineMeters(prev.coordinates,next.coordinates)/1000));
}

function formatHM(min){ const h=Math.floor(min/60), m=min%60; return `${String(h).padStart(2,'0')}h${String(m).padStart(2,'0')}m`; }

/* ===================== DnD helpers ===================== */
const DRAG_POI='TR_POI'; const DRAG_BASE='TR_BASE';
function makeDragData(type,p,fromDay,index){ return JSON.stringify({type,p,fromDay,index}); }
function readDragData(e){ try{ return JSON.parse(e.dataTransfer.getData(DRAG_POI)||e.dataTransfer.getData(DRAG_BASE)); }catch{ return null; } }
...
function ColumnHeader({dia,index}){
  ...
  return (
    <div className="rounded-t-lg text-white text-xs font-semibold" style={{background:color,padding:'6px 8px'}}>
      <div style={{display:'flex',justifyContent:'space-between',gap:8}}>
        <div>{new Date(dia.data).toLocaleDateString('pt-BR',{weekday:'short',day:'2-digit',month:'2-digit'})}</div>
        <div style={{opacity:.95}}>
          <span style={{marginRight:8}}>📏 {totalKm} km</span>
...
function TimeGrid(){
  const rows=[]; const cap=DAY_END_MIN-DAY_START_MIN;
  for(let i=0;i<=cap;i+=30){
    const top=i/MINUTES_PER_PIXEL;
    const labelMin=DAY_START_MIN+i, h=Math.floor(labelMin/60), m=labelMin%60;
    const label=`${String(h).padStart(2,'0')}:${String(m).padStart(2,'0')}`;
    const isLast=i===cap;
    rows.push(
      <div key={i} style={{position:'absolute',top,left:0,right:0,height:0,borderTop:'1px dashed #e5e7eb'}}>
        <div style={{position:'absolute',left:4,top:isLast?-18:-8,fontSize:10,color:'#9ca3af',background:'#fff',padding:'0 4px'}}>{label}</div>
      </div>
    );
  }
  return <>{rows}</>;
...
function Card({ dataIndex,itemIndex,item,onResize,onRemove,allowDragBase,dayColor }){
  ...
  function onMouseDown(e){
    draggingRef.current=true; startYRef.current=e.clientY; startHRef.current=height;
  ...
  const bgTint=rgba(dayColor,.10), border=dayColor;
  const labelPrefix = isBase
    ...
    <div
      ...
      onDragStart={(e)=>{
        ...
        const p0={
          ...
          startAtMin:(typeof item.startAtMin==='number')?item.startAtMin:Math.max(0,item.start-DAY_START_MIN),
      ...
      style={{
        ...
        background:bgTint, border:`1px solid ${border}`,
      ...
      title={ isBase
        ...
        : 'Ponto de interesse' }
    >
      {/* faixa transparente para ver o horário de início */}
      <div style={{height:TOP_STRIPE_PX}} />

      {/* cabeçalho sólido (título) */}
      <div style={{
        background:'#fff', padding:'8px 8px 6px 8px',
        ...
        {!isBase && (
          <button onClick={(e)=>{e.stopPropagation();onRemove();}}
                  ...
                  title="Remover do dia">×</button>
        )}
      </div>

      {/* corpo (translúcido) */}
      <div style={{padding:'8px 8px 22px 8px'}}>
        {(item.travelBefore>0 || item.travelKmBefore>0) && (
...
export function TravelPlannerBoard({
  ...
  const dayColors=React.useMemo(()=>local.map((_,i)=>colorForDay(i)),[JSON.stringify(local)]);

  // sempre redesenha quando o conteúdo/cores mudam
  React.useEffect(()=>{ notifyMapFull(local,dayColors,onRebuildRoutes); },[JSON.stringify(local),JSON.stringify(dayColors)]);

  // 1ª abertura: força colorização imediata (vários pulsos p/ pegar o mapa)
  React.useEffect(()=>{
    const colors=local.map((_,i)=>colorForDay(i));
    const pulse=()=>{
      notifyMapFull(local,colors,onRebuildRoutes);
      window.dispatchEvent(new Event('tt:please-redraw'));
    };
    pulse();
    const t1=setTimeout(pulse,120);
    const t2=setTimeout(pulse,300);
    const t3=setTimeout(()=>window.TT_ON_BOARD_OPEN?.(),450);
  ...
  function dropOnDay(dayIndex,e){
    ...
    const rect=timelineRefs.current[dayIndex]?.getBoundingClientRect();
    ...
    const removeIfFromBoard=()=>{
      if(data.fromDay!==null && data.index!==null){
        return next[data.fromDay].pontos.splice(data.index,1)[0];
      }
      return null;
    };
    ...
    if(data.type===DRAG_BASE){
      ...
      // topo = início do dia (60 min)
      if(y<=40){
        next[dayIndex].pontos[0]={ ...base, tipo:'base', fixo:true, id:`base-start-${next[dayIndex].data}`, tempo:BASE_START_MIN, startAtMin:0 };
        if(dayIndex>0){
          const prev=next[dayIndex-1];
          prev.pontos[prev.pontos.length-1]={ ...base, tipo:'base', fixo:false, id:`base-end-${prev.data}`,
            tempo:BASE_END_MIN, startAtMin:Math.max(0, DAY_END_MIN-DAY_START_MIN-BASE_END_MIN) };
        }
        commit(next); return;
      }

      // rodapé = término (cola até 00:00 com 120 min)
      const capPx=((DAY_END_MIN-DAY_START_MIN)/MINUTES_PER_PIXEL);
      if(y>=(capPx-70)){
        next[dayIndex].pontos[next[dayIndex].pontos.length-1]={ ...base, tipo:'base', fixo:false, id:`base-end-${next[dayIndex].data}`,
          tempo:BASE_END_MIN, startAtMin:Math.max(0, DAY_END_MIN-DAY_START_MIN-BASE_END_MIN) };
        if(dayIndex<next.length-1){
          next[dayIndex+1].pontos[0]={ ...base, tipo:'base', fixo:true, id:`base-start-${next[dayIndex+1].data}`, tempo:BASE_START_MIN, startAtMin:0 };
        }
        commit(next); return;
      }

      // meio do dia = base intermediária (120 min) respeitando horário
      const sched=scheduleDay(next[dayIndex]);
      let idx=sched.findIndex(s=>(s.start-DAY_START_MIN)>desiredStartMin);
      if(idx<0) idx=next[dayIndex].pontos.length;
      idx=Math.max(1,idx);
      ...
      next[dayIndex].pontos.splice(idx,0,{
        ...
        tempo:BASE_MID_MIN, startAtMin:desiredStartMin
      });
      commit(next); return;
    }
  }
  ...
  function resizeItem(dayIndex,itemIndex,minutes){
    const next=clone(local);
    const it=next[dayIndex].pontos[itemIndex]; if(!it) return;

    if(it.tipo==='base' && (it.id?.startsWith('base-end-'))){
      const dur=Math.max(BASE_END_MIN, Math.round(minutes/RESIZE_STEP_MIN)*RESIZE_STEP_MIN);
      it.tempo=dur;
  ...
  function BaseChip({p}){
    return (
      <div draggable
           onDragStart={(e)=>{ e.dataTransfer.setData(DRAG_BASE,makeDragData(DRAG_BASE,{...p,tipo:'base'},null,null)); e.dataTransfer.effectAllowed='copyMove'; }}
  ...
  function PoiChip({p}){
    return (
      <div draggable
           onDragStart={(e)=>{ e.dataTransfer.setData(DRAG_POI,makeDragData(DRAG_POI,{...p,tipo:'interesse'},null,null)); e.dataTransfer.effectAllowed='copyMove'; }}
           className="px-2 py-1 border rounded text-xs bg-white hover:bg-gray-50 cursor-grab"
    ...
    <div className="w-full flex gap-4">
      ...
      <section className="flex-1 overflow-x-auto">
            ...
            return (
              <div key={dia.data} className="rounded-lg border border-gray-300 bg-white overflow-hidden flex flex-col">
                ...
                >
                  ...
                  {scheduled.map((s,i)=>(
                    <Card key={`${dia.data}-${i}-${s.id ?? s.uid ?? s.label}`}
                          dataIndex={dayIndex}
                          itemIndex={i}
                          item={s}
        ...
        <div className="text-xs text-gray-600 mt-3 leading-relaxed">
          <b>Dica:</b> arraste os cards para o <i>horário exato</i>. Bases: <b>topo</b> = início (60 min) • <b>rodapé</b> = término (>= 120 min, colado até 00:00) • <b>meio</b> = base intermediária (120 min). Para “chegar mais cedo”, <b>estique</b> o card de término para cima.
        </div>
      </section>
    </div>
  );
